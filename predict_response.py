...
def preprocess_user_input(user_input):
    ...
    stem_input = nltk.word_tokenize(user_input)
    # convert the user input into its root words : stemming
    stemmed_user_input = get_stem_words(stem_input,ignore_words)
    # Remove duplicacy and sort the user_input
    stemmed_user_input = sorted(list(set(stemmed_user_input)))

   
    # Input data encoding : Create BOW for user_input
    for word in words:
        if word in stem_input:
          bag_of_words.append(1)
        if word in stemmed_user_input:
...
def bot_response(user_input):
   ...
   # extract the class from the predicted_class_label (NEEDSREVIEW)
   predicted_class = classes.filter(predicted_class_label)
   ...
   for intent in intents['intents']:
    if intent['tag']==predicted_class:
       ...
       # choose a random bot response
        bot_response = intents.get(random.choice("responses"))
